...
% myodbc is your data source name
% user(root) change root to your username root that set in data source
% password('') fill the password that set in data source
% alias(store_db) alias store_db that will be used to call in other function (do not change this)
% open(once) it just run once when it opened

connect_to_db :-
    odbc_connect('myodbc', _,
                 [user(root), password(''), alias(store_db), open(once)]).
...
submit_store_handler(Request) :-
    ...
    format(atom(Query), 'INSERT INTO store (code, name, owner_name, description, address, category) VALUES (\'~w\', \'~w\', \'~w\', \'~w\', \'~w\', \'~w\')', [Code, Name, OwnerName, Description, Address, Category]),

    % Execute the SQL query
    catch(
        odbc_query(store_db, Query),
        Error,
        handle_error(Error)
    ),
...
submit_edit_store_handler(Request) :-
    ...
    ->  
        ...
        format(atom(Query), 'UPDATE STORE set name = \'~w\', owner_name = \'~w\', description = \'~w\', address = \'~w\', category = \'~w\' WHERE code = \'~w\' ', [Name, OwnerName, Description, Address, Category, Code]),
...
submit_store_review_handler(Request) :-
    ...
    % Construct the SQL query for insertion
    format(atom(Query), 'INSERT INTO store_review (store_code, name, description, rating) VALUES (\'~w\', \'~w\', \'~w\', \'~w\')', [StoreCode, Name, Description, Rating]),
